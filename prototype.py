...
def workflow(update):
    ...
    if text == '/add':
        sql.replace(table='user', data={'chat_id': chat_id, 'state_id': StateId.ADD_ADDMSG.value})
        add_person(chat_id, text)

    elif text == '/delete':
        sql.replace(table='user', data={'chat_id': chat_id, 'state_id': StateId.DEL_DELMSG.value})
        delete_person(chat_id, text)

    elif text == '/month':
        sql.replace(table='user', data={'chat_id': chat_id})
        response = this_month(chat_id)
        if response:
            send_msg(chat_id, response)
        else:
            send_msg(chat_id, "No one celebrates birthday this month")

    elif text in ('/list', '/all'):
        sql.replace(table='user', data={'chat_id': chat_id})          
        response = list_all(chat_id)
        if response:
            send_msg(chat_id, response)
        else:
...
def add_person(chat_id, text):
    ...
    if state_id == StateId.ADD_ADDMSG.value: # add message has come
        ...
        sql.update(table='user', set={'state_id': StateId.ADD_PERSNAME.value}, where={'chat_id':chat_id})
        ...
        sql.update(table='user', set={'state_id':StateId.ADD_PERSBDAY.value, 'state_data':json.dumps(state_data)}, where={'chat_id':chat_id})
        send_msg(chat_id, f"OK, now enter {state_data['pers_name']}\'s birthday (YYYY-MM-DD)")
...
def delete_person(chat_id, text):
    ...
    if state_id == StateId.DEL_DELMSG.value: # delete message has come
        ...
        people_list = sql.select_all(table='person', where={'chat_id': chat_id}, columns=['rowid as pers_id', '*']) # [{pers_name, pers_bday, chat_id, pers_id}, ...]
        ...
        list_to_show = '\n'.join([f"{item['ord_num']} {item['pers_name']} {item['pers_bday']}" for item in people_list])

        # Incremente state_id and send a message with result
        sql.update(table='user', set={'state_id': StateId.DEL_ORDNUM.value, 'state_data':json.dumps(people_list)}, where={'chat_id': chat_id})
        send_msg(chat_id, f"Enter a number of a person you want to delete:\n{list_to_show}")
        ...
        # Delete cache and send final message
        sql.replace(table='user', data={'chat_id': chat_id})
        send_msg(chat_id, f"{person_to_delete['pers_name']} born {person_to_delete['pers_bday']} has been deleted")
    else:
        sql.replace(table='user', data={'chat_id': chat_id})
        show_start_msg(chat_id)

def this_month(chat_id):
    log_msg("MONTH PROCESS HAS STARTED")
    month = dt.now().strftime("%m")
    result = sql.select_all(table='person', where={"chat_id": chat_id, "STRFTIME('%m', pers_bday)": month})
    if len(result) == 0:
        return None
    else:
...
def show_start_msg(chat_id):
    log_msg("START_MSG PROCESS HAS STARTED")
    msg = "Command list: \n/add - add a person \n/delete - delete a person \n/month - show birthdays of this month \n/list or /all - show all the people you've added"
    send_msg(chat_id, msg)

def send_msg(chat_id, msg):
    print(f">>> id is: {chat_id},", f"msg is: {msg}")

def get_state_data(chat_id):
    state_data_str = sql.select_one(table='user', where={'chat_id':chat_id})['state_data']
...
def validate_update(update):
    ...
    if not msg['chat']['id'].isdigit():
        return "ID is not integer", False
    return None, True
